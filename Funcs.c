...
void Deposit(BankAccount *h, int Number, float Amount, Date D) {
  ...
  if (p == NULL) { // first if
    ...
    while (Number != p->Number && p != NULL) { // get to the last node
      p = next_Account(p);
    }
    if (p ==
        NULL) { // we check if the number associated with the account exsists
      printf(" Sorry , This Account Does Not Exist . ");
    } else { // second
      p->Balance = p->Balance + Amount;
      Transaction *New_A;
      CreateNewTran(&New_A); // to put the new operation in this node
...
void Withdrawal(BankAccount *h, int Number, float Amount,
  ...
  if (p == NULL) {
    ...
    if (p == NULL) {
      ...
      if (p->Balance >=
          Amount) { // if the amount is less we can not do this opperation so
                    // this this condition is important
        p->Balance = p->Balance - Amount;
        Transaction *New_A;
...
void TransferAmount(BankAccount *h, int N1, int N2, float Amount,
  ...
  if (p == NULL) { // in case 0 acc
    printf(" you can not transfer any Amount ,  this opperation needs at least "
           "2 account ");
  } else { // first
    ...
    if (p == NULL) {
      printf(
          " no such account , you  can  not do any transfer "); // in case the
                                                                // account does
                                                                // not exist we
                                                                // do not need
                                                                // to find the
      ...
      if (q == NULL) {
        printf(
            " error !!this ooperation needs at least 2 accounts "); // in case
                                                                    // the
                                                                    // account
                                                                    // does
                                                                    // exist
...
int CheckAmount(BankAccount *h, int Number) { // begin of the function
  ...
  p = h;
  if (p == NULL) {
    printf("Soory , This ooperation can not happen it needs at least 1 "
           "Accounts . ");
  } else {
    createAccount(&p);
    while (Number != p->Number && p != NULL) {
...
while ( Number != p->Number &&  p != NULL)  {
  ...
  Temp_B = p -> transactionList ; // to get the head of the opeerations
  if ( Temp_B == NULL ){    
    AssignListToBank(p , New_A); // to link list of opperation with the accounts 
    AssignTranList(New_A,NULL);
    DeleteTran(New_A);
    }else { // third 
      while ( next_Transaction(Temp_B) != NULL ){
...
q= h;  // same 
  ...
  if ( p == NULL){
    printf (" no such account , Operation uncompleted it needs at least 2 accounts  "); // in case the account does not exist we do not need to find the other account  
                                             }else {
    while ( N2 != q-> Number && q != NULL){ // find the second  acc 
      p = next_Account(p);
    }
    if ( q == NULL)
     {
      printf (" error !!this operation needs at least 2 accounts "); // in case the account does exist 
     }else {
...
>>>>>>> 2da4676c18d2c4568c3785e13802c4a275b0f8e8
...
<<<<<<< HEAD
  ...
  case 4:
    if (h == NULL) {
      ...
      while (next_Account(temp) != NULL && c_num != temp->Number) {
